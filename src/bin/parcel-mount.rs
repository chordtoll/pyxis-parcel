...
fn remap_inode(pid: u32, ino: u32, name: String, parent: u64) -> u64 {
    ...
    }
    {
        ...
        for (pid, iid) in inomap.get(&parent).unwrap().clone() {
            if let Some(i) = parcels
                .get(pid as usize)
                .unwrap()
                .0
                .lookup(iid as u64, name.clone())
            {
...
fn remap_dirent(
    ...
    kind: FileType,
    name: String,
) -> (u64, i64, FileType, String) {
    (
        remap_inode(pid as u32, ino as u32, name.clone(), parent),
        offset,
        kind,
        name,
    )
...
impl Filesystem for PyxisFS {
        ...
        let mut pcid = None;
        {
            ...
            for (pid, iid) in inomap.get(&parent).unwrap().clone() {
                if let Some(i) = parcels
                    ...
                    .unwrap()
                    .0
                    .lookup(iid as u64, name.to_os_string().into_string().unwrap())
                {
    ...
    fn readdir(
        ...
        dirents.insert(
            ".".to_string(),
            (u32::MAX, (ino, InodeKind::Directory, ".".to_string())),
        );
        if let Some(p) = get_parent(ino) {
            dirents.insert(
                "..".to_string(),
                (u32::MAX, (p, InodeKind::Directory, "..".to_string())),
            );
        }
    ...
    fn read(
        ...
        if DEBUG {
            println!("read({:x},{},{},{})", ino, fh, offset, size);
        }
        let pcid;
        let inid;
        {
        ...
        let data = parcel
            .read(pfile, inid as u64, offset as u64, Some(size as u64))
            .unwrap();
        reply.data(&data);
    }
    fn readlink(&mut self, _req: &Request<'_>, ino: u64, reply: ReplyData) {
        if DEBUG {
            println!("readlink({:x})", ino);
        }
        let pcid;
        let inid;
        {
    ...
    fn getxattr(
        ...
        if DEBUG {
            println!("getxattr({:x},{:?},{})", ino, name, size);
        }
        let xattrs;
        {
            let parcels = &mut PARCELS.lock_mut().unwrap();
...
fn main() {
    ...
    if let Ok(Fork::Child) = daemon(true, true) {
        ...
        let options = vec![
            MountOption::FSName("pyxis-parcel".to_string()),
            MountOption::RO,
        ];
